from ConfigGenrator import GenerateConfigurationFile
import eel
...
# ACTS Function Begins

@eel.expose
def btn_checkxlsxGenConfigOnly(filename, ConfigTemplateFilePath, ConfigOutputFolder  ,fmghost, fmguser, fmgpasswd, fmgadom):

    return_html = ""

    print('Generating Configuration File ...')
    GenerateConfigurationFile(filename, ConfigTemplateFilePath, ConfigOutputFolder )

    print('File generation completed.')

    return_html += "<br>\n<b> >> Generating Configuration Files for {}.... <br>\n".format(filename)
    sendupdate(return_html)
    return_html += "<br>\n<b> >> Complete! <br>\n"
    return_html += "<br>\n <a href=\"ztptool_adv.html\">Return</a> <br>\n"
...
@eel.expose
def btn_easyDeploy(filename, ConfigTemplateFilePath, ConfigOutputFolder  ,fmghost, fmguser, fmgpasswd, fmgadom):
    ...
    ## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ### Step 1: Generate Config Files in json format
    
    logging.info("INFO:FileName= {}".format(filename))
    logging.info("INFO:ConfigTemplateFilePath= {}".format(ConfigTemplateFilePath))
    logging.info("INFO:ConfigOutputFolder= {}".format(ConfigOutputFolder))
    logging.info("INFO:fmghost= {}".format(fmghost))
    logging.info("INFO:fmguser= {}".format(fmguser))    

    logging.info("")
    logMessage = "INFO:Step-1: Generating Configuration File(s) ..."
    logging.info("")
    logging.info(logMessage)        
    print(logMessage)
    
    return_html += "<br>\n<b> >> Step 1: Generating Configuration Files for {}.... <br>\n".format(filename)
    sendupdate(return_html)
    ConfigFiles = GenerateConfigurationFile(filename, ConfigTemplateFilePath, ConfigOutputFolder )
    return_html += "<br>\n<b> >> 'Step 1' Complete! <br>\n"
    sendupdate(return_html)
    
    logMessage='Step 1, Config File(s) generation completed.'
    print(logMessage)
    logMessage = "INFO:{}".format(logMessage)
    logging.info("")


    ### >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ### Step 2: Import ADOM using generated ADOM files

    logging.info("")
    logMessage='Step-2: Importing ADOM'
    print(logMessage)
    logMessage = "INFO:{}".format(logMessage)
    logging.info(logMessage)
    

    return_html += "<br>\n<b> >> Step 2, Importing ADOM using Config File(s).... <br>\n"
    sendupdate(return_html)

    for configFile in ConfigFiles:
        logging.info("Import ADOM for Config File {}".format(configFile) )
        btn_checkadom(configFile, fmghost, fmguser, fmgpasswd, fmgadom, "" )


    logging.info("")
    logMessage='Step 2, Importing ADOM Completed!'
    print(logMessage)
    logMessage = "INFO:{}".format(logMessage)
    logging.info(logMessage)
    ...
    return_html += "<br>\n<b> >> 'Step 2' Complete! <br>\n"

    ### >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ### Step 3: Import Devices using input Excel files
    
    logging.info("")
    logMessage='Step 3: Importing Devices'
    print(logMessage)
    logMessage = "INFO:{}".format(logMessage)
    logging.info(logMessage)
    

    result=btn_checkxlsx(filename, fmghost, fmguser, fmgpasswd, fmgadom)


    logging.info("")
    logMessage='Step 3 Completed!'
    print(logMessage)
    logMessage = "INFO:{}".format(logMessage)
    logging.info(logMessage)
...
def btn_checkxlsx(filename, fmghost, fmguser, fmgpasswd, fmgadom):
    ...
    if alldevices == "workbook":
        return_html += "Load Excel workbook failed-1 <span class=\"glyphicon glyphicon-remove\" style=\"color:red\"></span><br>\n"
    ...
    if proceed == True:
        ...
        required_headings = ["Device_Name", "Platform", "Device_SN", "CLI_Template", "Device_Name",
                             "Policy_Package",
                             "SDWAN_Template"]
    ...
    if proceed == True:
        ...
        for devicedata in alldevices:
            ...
            if qi_status == True:
                ...
                if devicedata['Device_Name'] == "" or devicedata['Device_Name'] is None:
                    return_html += "Assign Post CLI Template {not defined} <span class=\"glyphicon glyphicon-info-sign\" style=\"color:orange\"></span><br>\n"
                else:
                    status_clitempgrp = assign_cli_template(fmg_adom, devicedata['Device_Name'],
                                                            devicedata['Device_Name'])
                    if status_clitempgrp == "OK":
                        return_html += "Assign Post CLI Template \"" + devicedata[
                            'Device_Name'] + "\" successful <span class=\"glyphicon glyphicon-ok\" style=\"color:green\"></span><br>\n"
                    else:
                        return_html += "Assign Post CLI Template \"" + devicedata[
                            'Device_Name'] + "\" failed <span class=\"glyphicon glyphicon-remove\" style=\"color:red\"></span><br>\n"
...
def btn_checkadom(filename, fmghost, fmguser, fmgpasswd, fmgadom, fmgadomdesc):
    ...
    return_html += "<br>\n<b> >> Complete! <br>\n"
    return_html += "<br>\n <a href=\"ztptool_adv.html\">Return</a> <br><br><br><br>&nbsp;\n"
...
def btn_getxlsxfile():
    ...
    filename = filedialog.askopenfilename(initialdir="device_data/", title="Select file",
...
# --- ACTS Function Begins ---
@eel.expose
def btn_getconfigoutfolder():
    root = Tk()
    root.withdraw()
    root.wm_attributes('-topmost', 1)
    print("filename ....")
    folderPath = filedialog.askdirectory(initialdir="config_out/")
    root.update()  # to make dialog close on MacOS
    print(folderPath)
...
# --- ACTS Function Begins ---
@eel.expose
def btn_getConfigTemplateFilePath():
    root = Tk()
    root.withdraw()
    root.wm_attributes('-topmost', 1)

    filename = filedialog.askopenfilename(initialdir="config_templete/", title="Select template file",
                                          filetypes=(("Config Template Files", "*.conf") ,("all files", "*.*")))
    root.update()  # to make dialog close on MacOS
    print(filename)
...
def generate_Config():
    try:
        with open('settings.json') as json_settings:
            ...
            default_ConfigOutputPath = "/"
            default_ConfigTemplatePath = "/"
    ...
    <div>
      <form autocomplete="off">
        ...
        <div form-group>
          <button type="button" onclick="getFolder()" class="btn btn-secondary btn-sm">Select Input File</button>
          Excel Path: <span id="filepath">%s</span>

          </br> </br>

          <button type="button" onclick="getConfigTemplateFilePath()" class="btn btn-secondary btn-sm">Select Config Template File</button>
          Template Path: <span id="ConfigTemplateFilePath">%s</span>


          </br> </br>

          <button type="button" onclick="getConfigOutputFolder()" class="btn btn-secondary btn-sm">Select Config Output Folder</button>
          Output Path:  <span id="ConfigOutputFolder">%s</span>
        ...
        <div class="form-group">
          <button type="button" onclick="processxlsxGenConfig(document.getElementById('filepath').innerHTML, document.getElementById('ConfigTemplateFilePath').innerHTML, document.getElementById('ConfigOutputFolder').innerHTML )" class="btn btn-primary">Generate Config</button>
        </div>
      </form>

    </div>
          ''' % (default_fmg, default_user, default_passwd, default_adom, default_path, default_ConfigTemplatePath ,default_ConfigOutputPath)
...
def easy_deploy():
    ...
    <div>
      <form autocomplete="off">
        ...
        <div form-group>
          <button type="button" onclick="getFolder()" class="btn btn-secondary btn-sm">Select Devices Details File</button>
          Excel Path: <span id="filepath">%s</span>
        ...
        <div class="form-group">
          <button type="button" onclick="processEasyDeploy(document.getElementById('filepath').innerHTML, document.getElementById('ConfigTemplateFilePath').innerHTML, document.getElementById('ConfigOutputFolder').innerHTML )" class="btn btn-success">Start Deployment</button>
        </div>
      </form>
...
if use_mode == "":
    try:        
        eel.start('ztptool.html', size=(850, 850), disable_cache=True)
